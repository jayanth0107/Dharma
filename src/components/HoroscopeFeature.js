// Dharma Daily — Daily Horoscope (రాశి ఫలం) — Premium Feature
...
function getHoroscopeQrUrl(amount) {
  const upiStr = `upi://pay?pa=${UPI_ID_H}&pn=${MERCHANT_H}&am=${amount}&cu=INR&tn=DharmaDaily Horoscope`;
                        ...
                        <TouchableOpacity
                          ...
                          onPress={async () => {
                            if (Platform.OS === 'web') {
                              ...
                              const url = `${app.scheme}://upi/pay?pa=${encodeURIComponent(UPI_ID_H)}&pn=${encodeURIComponent(MERCHANT_H)}&am=${selectedPlan.price}&cu=INR&tn=${encodeURIComponent('DharmaDaily Horoscope ' + selectedPlan.english)}`;
...
function buildHoroscopeHtml(h) {
  return `<!DOCTYPE html><html><head><meta charset="utf-8">
    ...
    <div class="footer">Generated by ధర్మ దినచర్య (Dharma Daily) App<br>Vedic calculations using Drik Ganita + Lahiri Ayanamsa<br>🙏 సర్వే జనాః సుఖినో భవంతు</div>
    </body></html>`;
}

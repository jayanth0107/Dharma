...
export function SectionShareRow({ buildText, section, insideModal, autoOpen, onClose, hideButton }) {
  ...
  const handlePdf = async () => {
    ...
    // Fallback PDF used by sections that don't have a custom builder
    // (panchangam-share, festivals share, etc.). Mirrors the brand /
    // typography of the dedicated PDFs (matchmaking, horoscope, muhurtam)
    // — saffron+gold accents, 17 px body floor, section divider.
    ...
    const html = `<!DOCTYPE html><html lang="te"><head><meta charset="utf-8"/>
      <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .title-page {
          ...
          padding: 28px 22px; text-align: center; margin-bottom: 22px;
        }
        .brand-flag { font-size: 34px; line-height: 1.2; }
        .brand-name { font-size: 26px; font-weight: 700; color: #E8751A; margin-top: 4px; }
        .brand-tag { font-size: 13px; color: #D4A017; letter-spacing: 2px; margin-top: 4px; }
        .rule { width: 60px; height: 2px; background: #D4A017; margin: 18px auto; }
        .section-name { font-size: 20px; font-weight: 700; color: #2C1810; }
        .meta { font-size: 14px; color: #8A6A40; margin-top: 8px; }
        .section-head {
        ...
        .section-title { font-size: 20px; font-weight: 700; color: #E8751A; }
        .content {
          white-space: pre-wrap;
          font-family: inherit; font-size: 17px; line-height: 1.75;
          background: #FAF6F0; color: #2C1810;
          padding: 20px 22px; border-radius: 12px;
          border: 1px solid #E8D8C0; border-left: 4px solid #E8751A;
        }
        .footer { text-align: center; margin-top: 22px; padding-top: 14px; border-top: 1px solid #E8D8C0; }
        .footer-brand { font-size: 15px; font-weight: 700; color: #D4A017; }
        .footer-sub { font-size: 13px; color: #8A6A40; margin-top: 4px; }
        .footer-link { font-size: 12px; color: #999; margin-top: 8px; }
      </style>
      </head><body>
      ...
      <div class="section-head">
        ...
        <span class="section-title">వివరాలు — Details</span>
      </div>
      <div class="content">${safeText}</div>
      <div class="footer">
        <div class="footer-brand">🙏 సర్వే జనాః సుఖినో భవంతు</div>
        <div class="footer-sub">Generated by ధర్మ App — Telugu Panchangam &amp; Astro</div>
        <div class="footer-link">📲 play.google.com/store/apps/details?id=com.dharmadaily.wisdom</div>
      </div>
      </body></html>`;
...
const s = StyleSheet.create({
  ...
  // Consent — gap/padding bumped to match the rest of the modal's
  // 12/16 rhythm (matches Spacing.md / Spacing.lg tokens).
  consentRow: {
  ...
  // Fonts here follow the Type token floor — caption=13, small=15 —
  // since the share modal is one of the most-shared surfaces and the
  // earlier 11-12 pt strip felt hard to read on tester phones.
  consentText: { flex: 1, fontSize: 13, color: '#4CAF50', lineHeight: 19, fontWeight: '500' },

  // Platforms
  platformLabel: { fontSize: 15, fontWeight: '700', color: DarkColors.textPrimary, marginBottom: 10, letterSpacing: 0.3 },
  platformList: {},
  platformBtn: {
  ...
  platformTelugu: { fontSize: 13, color: DarkColors.textMuted, marginTop: 2 },
  platformHint: { fontSize: 13, color: DarkColors.textMuted, marginRight: 6, fontWeight: '500' },

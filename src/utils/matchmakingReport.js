    ...
    return `
      <tr style="background:${rowBg};">
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #2A2A2A;">
          ...
          <span style="color:#9A9A9A;font-size:17px;">${escapeHtml(k.nameEn)}</span>
        </td>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #2A2A2A;text-align:center;">
          <span style="color:${scoreColor};font-weight:700;font-size:18px;">${k.score}</span>
        </td>
        <td style="padding:10px 12px;border-bottom:1px solid #2A2A2A;color:#9A9A9A;font-size:17px;">
  ...
  const kutaInterpretations = (result.kutas || []).map((k, i) => {
    ...
    const extSections = ext.whatItMeasures ? `
      <div style="margin-top:12px;padding-top:12px;border-top:1px solid #2A2A2A;">
        <p style="color:#D4A017;font-size:17px;font-weight:700;margin-bottom:4px;">&#128269; ${escapeHtml(ext.areaOfLife?.en || '')}</p>
        <p style="color:#ccc;font-size:17px;line-height:1.6;margin:4px 0;">${escapeHtml(ext.whatItMeasures.te)}</p>
        <p style="color:#9A9A9A;font-size:16px;line-height:1.5;margin:2px 0;">${escapeHtml(ext.whatItMeasures.en)}</p>
      </div>
      <div style="margin-top:10px;">
        <p style="color:#D4A017;font-size:17px;font-weight:700;margin-bottom:4px;">&#9881; How It Works</p>
        <p style="color:#ccc;font-size:17px;line-height:1.6;margin:4px 0;">${escapeHtml(ext.howItWorks.te)}</p>
        <p style="color:#9A9A9A;font-size:16px;line-height:1.5;margin:2px 0;">${escapeHtml(ext.howItWorks.en)}</p>
      </div>
      <div style="margin-top:10px;">
        <p style="color:#D4A017;font-size:17px;font-weight:700;margin-bottom:4px;">&#10084; Impact on Marriage</p>
        <p style="color:#ccc;font-size:17px;line-height:1.6;margin:4px 0;">${escapeHtml(ext.impactOnMarriage.te)}</p>
        <p style="color:#9A9A9A;font-size:16px;line-height:1.5;margin:2px 0;">${escapeHtml(ext.impactOnMarriage.en)}</p>
      </div>
      <div style="margin-top:10px;background:rgba(212,160,23,0.06);border-radius:8px;padding:10px 12px;">
        <p style="color:#E8751A;font-size:17px;font-weight:700;margin-bottom:4px;">&#128737; Remedy</p>
        <p style="color:#ccc;font-size:17px;line-height:1.6;margin:4px 0;">${escapeHtml(ext.remedy.te)}</p>
        <p style="color:#9A9A9A;font-size:16px;line-height:1.5;margin:2px 0;">${escapeHtml(ext.remedy.en)}</p>
      </div>` : '';
    return `
      <div class="interp-card" style="background:#111;border:1px solid #2A2A2A;border-radius:8px;padding:16px 18px;margin-bottom:16px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
          <span style="color:${statusColor};font-size:20px;font-weight:700;">${statusIcon}</span>
          <span style="color:#D4A017;font-size:18px;font-weight:700;">${i + 1}. ${escapeHtml(k.name)} (${escapeHtml(k.nameEn)})</span>
          <span style="margin-left:auto;color:${statusColor};font-weight:700;font-size:20px;">${k.score}/${k.max}</span>
        </div>
        ${barHtml}
        ${interp.telugu ? `<p style="color:${statusColor};margin:6px 0;line-height:1.6;font-size:17px;font-weight:600;">${escapeHtml(interp.telugu)}</p>` : ''}
        ${interp.english ? `<p style="color:#9A9A9A;margin:4px 0;line-height:1.5;font-size:17px;font-style:italic;">${escapeHtml(interp.english)}</p>` : ''}
        ${extSections}
      </div>`;
  }).join('');
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body {
    ...
    margin: 0 auto;
    padding: 20px 16px;
    /* PDF baseline bumped 2026-05-16 — previously unset (browser default
       ~13–14px on most expo-print renderers). 16px body with 1.65 line-
       height is the readable floor on a phone PDF viewer. */
    font-size: 18px;
    line-height: 1.65;
...
<div class="title-page" style="background:linear-gradient(135deg,#0A0A0A 0%,#1A1000 50%,#0A0A0A 100%);border:2px solid #D4A017;border-radius:12px;padding:40px 24px;text-align:center;margin-bottom:30px;">
  ...
  <p style="color:#D4A017;font-size:17px;letter-spacing:2px;margin-bottom:24px;">DHARMA: TELUGU ASTRO, CALENDAR &amp; GOLD</p>
  <div style="width:60px;height:2px;background:#D4A017;margin:0 auto 24px;"></div>
  <h2 style="color:#fff;font-size:24px;margin-bottom:6px;">జాతక పొందిక నివేదిక</h2>
  <h3 style="color:#D4A017;font-size:19px;font-weight:500;margin-bottom:20px;">Horoscope Matching Report</h3>
  <p style="color:#9A9A9A;font-size:17px;">అష్టకూట కుండలి మిలన్ — Ashtakoota Kundali Milan</p>
  <div style="margin-top:28px;display:flex;justify-content:center;gap:40px;flex-wrap:wrap;">
    <div>
      <p style="color:#9A9A9A;font-size:16px;margin-bottom:4px;">వరుడు (Groom)</p>
      <p style="color:#D4A017;font-size:20px;font-weight:700;">${gName}</p>
    </div>
    <div style="color:#D4A017;font-size:24px;align-self:center;">&#10084;</div>
    <div>
      <p style="color:#9A9A9A;font-size:16px;margin-bottom:4px;">వధువు (Bride)</p>
      <p style="color:#D4A017;font-size:20px;font-weight:700;">${bName}</p>
    </div>
  </div>
  <p style="color:#555;font-size:16px;margin-top:20px;">Report generated on ${escapeHtml(reportDate)}</p>
</div>

<!-- ══════════════ BIRTH DETAILS ══════════════ -->
<div class="section-card" style="background:#111;border:1px solid #2A2A2A;border-radius:10px;padding:20px;margin-bottom:24px;">
  <h2 style="color:#E8751A;font-size:20px;margin-bottom:16px;padding-bottom:8px;border-bottom:1px solid #2A2A2A;">
    &#128197; జన్మ వివరాలు — Birth Details
  </h2>
  <table>
    <thead>
      <tr style="background:#1A1000;">
        <th style="padding:10px 12px;text-align:left;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">Detail</th>
        <th style="padding:10px 12px;text-align:center;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">&#9794; వరుడు (Groom)</th>
        <th style="padding:10px 12px;text-align:center;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">&#9792; వధువు (Bride)</th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr style="background:#141414;">
        ...
        <td style="padding:10px 12px;text-align:center;color:#D4A017;border-bottom:1px solid #2A2A2A;font-weight:600;">
          ${escapeHtml(result.groomNakshatra?.telugu || '')} <span style="color:#9A9A9A;font-size:16px;">(${escapeHtml(result.groomNakshatra?.english || '')})</span>
        </td>
        <td style="padding:10px 12px;text-align:center;color:#D4A017;border-bottom:1px solid #2A2A2A;font-weight:600;">
          ${escapeHtml(result.brideNakshatra?.telugu || '')} <span style="color:#9A9A9A;font-size:16px;">(${escapeHtml(result.brideNakshatra?.english || '')})</span>
        </td>
      </tr>
      <tr style="background:#1A1A1A;">
        <td style="padding:10px 12px;color:#9A9A9A;border-bottom:1px solid #2A2A2A;">Rashi / రాశి</td>
        <td style="padding:10px 12px;text-align:center;color:#D4A017;border-bottom:1px solid #2A2A2A;font-weight:600;">
          ${escapeHtml(result.groomRashi?.telugu || '')} <span style="color:#9A9A9A;font-size:16px;">(${escapeHtml(result.groomRashi?.english || '')})</span>
        </td>
        <td style="padding:10px 12px;text-align:center;color:#D4A017;border-bottom:1px solid #2A2A2A;font-weight:600;">
          ${escapeHtml(result.brideRashi?.telugu || '')} <span style="color:#9A9A9A;font-size:16px;">(${escapeHtml(result.brideRashi?.english || '')})</span>
        </td>
      </tr>
      <tr style="background:#141414;">
        ...
        <td style="padding:10px 12px;text-align:center;color:#E0E0E0;">
          ${escapeHtml(RASHI_LORD_NAMES.te[groomRashiIdx])} <span style="color:#9A9A9A;font-size:16px;">(${escapeHtml(RASHI_LORD_NAMES.en[groomRashiIdx])})</span>
        </td>
        <td style="padding:10px 12px;text-align:center;color:#E0E0E0;">
          ${escapeHtml(RASHI_LORD_NAMES.te[brideRashiIdx])} <span style="color:#9A9A9A;font-size:16px;">(${escapeHtml(RASHI_LORD_NAMES.en[brideRashiIdx])})</span>
        </td>
      </tr>
    </tbody>
  </table>
...
<div class="section-card" style="background:#111;border:1px solid #2A2A2A;border-radius:10px;padding:20px;margin-bottom:24px;">
  ...
  <table>
    <thead>
      <tr style="background:#1A1000;">
        <th style="padding:10px 12px;text-align:left;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">లక్షణం / Attribute</th>
        <th style="padding:10px 12px;text-align:center;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">&#9794; ${gName}</th>
        <th style="padding:10px 12px;text-align:center;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">&#9792; ${bName}</th>
      </tr>
    </thead>
    <tbody>
      ${[
        ...
        return '<tr style="background:' + bg + ';"><td style="padding:10px 12px;color:#9A9A9A;border-bottom:1px solid #2A2A2A;">' + escapeHtml(row.label) + '</td><td style="padding:10px 12px;text-align:center;color:#E0E0E0;border-bottom:1px solid #2A2A2A;font-weight:600;">' + escapeHtml(gp?.te || '') + ' <span style="color:#9A9A9A;font-size:16px;">(' + escapeHtml(gp?.en || '') + ')</span></td><td style="padding:10px 12px;text-align:center;color:#E0E0E0;border-bottom:1px solid #2A2A2A;font-weight:600;">' + escapeHtml(bp?.te || '') + ' <span style="color:#9A9A9A;font-size:16px;">(' + escapeHtml(bp?.en || '') + ')</span>' + (match ? ' <span style="color:#4CAF50;font-size:15px;">&#10004;</span>' : '') + '</td></tr>';
      }).join('')}
    </tbody>
  </table>
</div>` : ''}
...
<div class="section-card" style="background:#111;border:1px solid #2A2A2A;border-radius:10px;padding:20px;margin-bottom:24px;">
  ...
  <table>
    <thead>
      <tr style="background:#1A1000;">
        <th style="padding:10px 8px;text-align:center;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">#</th>
        <th style="padding:10px 12px;text-align:left;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">గుణం (Guna)</th>
        <th style="padding:10px 8px;text-align:center;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">Max</th>
        <th style="padding:10px 8px;text-align:center;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">Score</th>
        <th style="padding:10px 12px;text-align:left;color:#D4A017;font-size:17px;border-bottom:2px solid #D4A017;">Area of Life</th>
      </tr>
    </thead>
    <tbody>
      ${kutaRows}
      <tr style="background:#1A1000;border-top:2px solid #D4A017;">
        <td colspan="2" style="padding:12px;color:#D4A017;font-weight:700;font-size:18px;">
          మొత్తం — Total
        </td>
        <td style="padding:12px;text-align:center;color:#D4A017;font-weight:700;font-size:18px;">${result.maxScore}</td>
        <td style="padding:12px;text-align:center;color:${verdictBorder};font-weight:700;font-size:22px;">${result.totalScore}</td>
        <td style="padding:12px;color:#9A9A9A;font-size:17px;">${result.percentage}% match</td>
      </tr>
    </tbody>
  </table>
</div>
...
<div style="background:${verdictBg};border:2px solid ${verdictBorder};border-radius:10px;padding:24px;margin-bottom:24px;text-align:center;">
  <p style="color:#9A9A9A;font-size:17px;margin-bottom:8px;">Match Result / పొందిక ఫలితం</p>
  <div style="display:flex;align-items:center;justify-content:center;gap:16px;flex-wrap:wrap;margin-bottom:12px;">
    <div style="width:80px;height:80px;border-radius:50%;border:3px solid ${verdictBorder};display:flex;align-items:center;justify-content:center;">
      <span style="color:${verdictBorder};font-size:28px;font-weight:700;">${result.totalScore}<span style="font-size:17px;color:#9A9A9A;">/${result.maxScore}</span></span>
    </div>
    <div>
      <p style="color:${verdictBorder};font-size:20px;font-weight:700;margin-bottom:4px;">${result.percentage}% Compatibility</p>
      <p style="color:#E0E0E0;font-size:17px;font-weight:600;">${escapeHtml(result.verdict || '')}</p>
      <p style="color:#9A9A9A;font-size:17px;">${escapeHtml(result.verdictEn || '')}</p>
    </div>
  </div>
</div>
  ...
  <div style="display:flex;gap:20px;flex-wrap:wrap;justify-content:center;">
    ${[
      ...
      const cells = Array.from({length:12}, (_,i) => {
        const idx = (ri + i) % 12;
        const isMoon = i === 0;
        return `<td style="width:25%;padding:6px;text-align:center;border:1px solid #2A2A2A;font-size:15px;color:${isMoon ? '#D4A017' : '#9A9A9A'};font-weight:${isMoon ? '700' : '400'};background:${isMoon ? 'rgba(212,160,23,0.08)' : 'transparent'};">${rashiNames[idx]}${isMoon ? '<br/><span style="color:#D4A017;font-size:17px;">Mo</span>' : ''}</td>`;
      });
      return `<div style="flex:1;min-width:200px;">
        <p style="text-align:center;color:${p.color};font-weight:700;font-size:17px;margin-bottom:8px;">${p.icon} ${escapeHtml(p.name || '')}</p>
        <p style="text-align:center;color:#9A9A9A;font-size:16px;margin-bottom:8px;">${escapeHtml(p.nak?.telugu || '')} (${escapeHtml(p.nak?.english || '')}) · ${escapeHtml(p.rashi?.telugu || '')} (${escapeHtml(p.rashi?.english || '')})</p>
        <table style="width:100%;border-collapse:collapse;border:1px solid #2A2A2A;">
          <tr>${cells[0]}${cells[1]}${cells[2]}${cells[3]}</tr>
          <tr>${cells[11]}<td colspan="2" rowspan="2" style="text-align:center;border:1px solid #2A2A2A;padding:12px;"><span style="color:#D4A017;font-size:18px;font-weight:700;">${escapeHtml(p.rashi?.telugu || '')}</span><br/><span style="color:#9A9A9A;font-size:15px;">${escapeHtml(p.rashi?.english || '')}</span></td>${cells[4]}</tr>
...
<div class="section-card" style="background:#111;border:1px solid #2A2A2A;border-radius:10px;padding:20px;margin-bottom:24px;">
  ...
  <div style="display:flex;gap:16px;flex-wrap:wrap;">
    <div style="flex:1;min-width:200px;background:#141414;border-radius:8px;padding:16px;border-left:4px solid ${mangalColor(gMangal)};">
      <p style="color:#9A9A9A;font-size:16px;margin-bottom:6px;">&#9794; వరుడు — ${gName}</p>
      <p style="color:${mangalColor(gMangal)};font-size:18px;font-weight:700;margin-bottom:4px;">${escapeHtml(gMangal.telugu || '')}</p>
      <p style="color:#9A9A9A;font-size:17px;">${escapeHtml(gMangal.english || '')}</p>
    </div>
    <div style="flex:1;min-width:200px;background:#141414;border-radius:8px;padding:16px;border-left:4px solid ${mangalColor(bMangal)};">
      <p style="color:#9A9A9A;font-size:16px;margin-bottom:6px;">&#9792; వధువు — ${bName}</p>
      <p style="color:${mangalColor(bMangal)};font-size:18px;font-weight:700;margin-bottom:4px;">${escapeHtml(bMangal.telugu || '')}</p>
      <p style="color:#9A9A9A;font-size:17px;">${escapeHtml(bMangal.english || '')}</p>
    </div>
  </div>
  ...
  <div style="margin-top:12px;padding:12px;background:#1A1010;border-radius:6px;border:1px solid #2A1A1A;">
    <p style="color:#E8495A;font-size:17px;line-height:1.5;">
      &#9888; మంగళ దోషం ఉన్నట్లయితే, ఇద్దరిలో దోషం ఉంటే లేదా నివారణ పూజ చేయించినట్లయితే ప్రభావం తగ్గుతుంది.
    </p>
    <p style="color:#9A9A9A;font-size:16px;line-height:1.4;margin-top:4px;">
...
<div class="section-card" style="background:#111;border:1px solid #D4A017;border-radius:10px;padding:24px;margin-bottom:24px;">
  <h2 style="color:#E8751A;font-size:20px;margin-bottom:12px;">
    &#128220; ముగింపు — Conclusion
  </h2>
  <p style="color:#E0E0E0;line-height:1.7;font-size:17px;margin-bottom:10px;">
  ...
  <p style="color:#D4A017;font-size:17px;font-weight:600;margin-bottom:10px;">
    ${escapeHtml(result.verdict || '')}
  </p>
  <p style="color:#9A9A9A;font-size:17px;line-height:1.6;margin-bottom:8px;">
  ...
  <p style="color:#4CAF50;font-size:17px;margin-top:12px;">
    &#10004; ఈ జంట వివాహానికి అనుకూలమైన పొందిక కలిగి ఉంది. / This match is considered suitable for marriage.
  </p>` : `
  <p style="color:#E8495A;font-size:17px;margin-top:12px;">
